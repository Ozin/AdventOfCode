...
public class Day13 extends AbstractIntcodePuzzle {
    ...
    protected Object a(final long[] program) throws Exception {
        ...
        final long[] longs = intcodeComputer.finishProgram();
        final Map<Point, Long> gameState = mapOutput(longs);
        printGame(gameState);
    ...
    protected Object b(final long[] program) throws Exception {
        ...
        while (!intcodeComputer.isDone()) {
            final long[] longs = nextScreen(intcodeComputer);
    ...
    private long[] initScreen(final IntcodeComputer intcodeComputer) {
        final var res = new long[3120 - 9];
        for (int i = 0; i < res.length; i += 3) {
            final long[] tmp = nextScreen(intcodeComputer);
            res[i] = tmp[0];
            res[i + 1] = tmp[1];
            res[i + 2] = tmp[2];
        }
    ...
    private int adjustPaddle(final Map<Point, Long> gameState) {
        ...
        for (final Map.Entry<Point, Long> tile : gameState.entrySet()) {
    ...
    private long[] nextScreen(final IntcodeComputer intcodeComputer) {
        final long[] screen = new long[3];
        for (int i = 0; i < screen.length; i++) {
            final long output = intcodeComputer.nextOutput();
    ...
    public void printGame(final Map<Point, Long> gameState) {
        ...
        for (int y = minY; y <= maxY; y++) {
            for (int x = minX; x <= maxX; x++) {
                final int value = Math.toIntExact(gameState.getOrDefault(new Point(x, y), (long) IntcodeGame.TileType.EMPTY.ordinal()));
                final String stringValue = IntcodeGame.TileType.values()[value].toString();
    ...
    private Map<Point, Long> mapOutput(final long[] output) {
        ...
        for (int i = 0; i < output.length; i += 3) {
            final int y = (int) output[i + 1];
            final int x = (int) output[i];

            final Point key = new Point(x, y);
            final long value = output[i + 2];
