...
public class Day07 {
    ...
    private static Object one(final String[] input) {
        ...
        for (final var e : files) {
            final Set<String> parentFolders = findParentFolders(e._1.split("/"));
            for (final String parentFolder : parentFolders) {
    ...
    private static Set<String> findParentFolders(final String[] split) {
        Set<String> folders = HashSet.of();
        String current = "";
        for (int i = 0; i < split.length - 1; i++) {
            current = current + split[i] + "/";
            folders = folders.add(current);
        }
        return folders;
    }

    private static Map<String, Integer> getFiles(final String[] input) {
        final Pattern FILE_MATCH = Pattern.compile("(\\d+) (.+)");
        Map<String, Integer> files = HashMap.empty();

        var currentFolder = "";
        for (final String line : input) {
            final var fileMatch = FILE_MATCH.matcher(line);
            if (fileMatch.find()) {
                files = files.put(currentFolder + fileMatch.group(2), Integer.parseInt(fileMatch.group(1)));
            } else if (line.startsWith("$ cd ")) {
                currentFolder = cd(currentFolder, line.substring(5));
            }
        }
    ...
    private static String cd(final String currentFolder, final String subpath) {
        if (subpath.startsWith("/")) return subpath;
        if ("..".equals(subpath)) {
            final var parent = currentFolder.substring(
                    0,
